...
void *funct(void *argument)
{
    ...
    Mat frameCopy = args->frame1;
    int counters = args->count;
    float dens = args->dens;
    Mat imgDifference;
    Mat imgThresh;
    ...
    errorSquared += (dens - 1 + (float)ZeroPixels / TotalNumberOfPixels) * (dens - 1 + (float)ZeroPixels / TotalNumberOfPixels);
    pthread_exit(0);
}
