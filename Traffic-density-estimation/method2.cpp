...
void funct(Mat frameCopy, int counters)
{
    ...
    Mat frameResize, frame2Resize;
    resize(frameFinal, frameResize, Size(), shrink_factor, shrink_factor);
    resize(frame2Final, frame2Resize, Size(), shrink_factor, shrink_factor);
    absdiff(frameResize, frame2Resize, imgDifference);
...
int main(int argc, char **argv)
{
    ...
    if (argv[2] == NULL || stof(argv[2]) >= 1)
    {
        cout << "provide valid shrink_factor" << endl;
        exit(1);
    }
    shrink_factor = stof(argv[2]);
